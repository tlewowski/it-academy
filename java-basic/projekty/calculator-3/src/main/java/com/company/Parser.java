...
public class Parser {
    ...
    public Node operatorParser(String[] s) throws InstantiationException, IllegalAccessException {
        ...
        index++;
        Node left=makeTreeHelper(s);
        Node right=makeTreeHelper(s);
        op.setLeft(left);
        op.setRigth(right);
        //return new Add(left,right);
        return op;
    }
    Node makeTree(String[] s) throws IllegalArgumentException, InstantiationException, IllegalAccessException{
      Node tree = makeTreeHelper(s);
        if (s.length > index){
          throw new IllegalArgumentException();
      }
      return tree;
    }
    Node makeTreeHelper(String[] s) throws IllegalAccessException, InstantiationException {
